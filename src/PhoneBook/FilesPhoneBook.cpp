...
Tm_FilesPhoneBook::Tm_State *Tm_FilesPhoneBook::On_ACTIVE() {
    ATE_SWITCH_PRIM() {
        ...
        TTRACE(5, "Got Request to add contact");
        En_ResultCode result = AddContact(p->m_Contact);
        switch (result) {
            case En_ResultCode::Ok:
                m_pOutput->Put(new Pr_ADD_CONTACT_CONF(result));
                break;
            default:
        ...
        TTRACE(5, "Got Request to edit contact");
        En_ResultCode result = EditContact(p->m_Contact);
        switch (result) {
            case En_ResultCode::Ok:
                m_pOutput->Put(new Pr_EDIT_CONTACT_CONF(result));
            break;
            default:
        ...
        TTRACE(5, "Got Request to remove contact");
        En_ResultCode result = RemoveContact(p->m_ContactId);
        switch (result) {
            case En_ResultCode::Ok:
                m_pOutput->Put(new Pr_REMOVE_CONTACT_CONF(result));
            break;
            default:
        ...
        TTRACE(5, "Got Request to print contact");
